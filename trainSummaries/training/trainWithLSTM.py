...
# Load FinBERT model (with optional freezing)
def load_finbert_model(freeze=True):
...
# Prepare and train on each window
def process_window(window_name, technical_data, textual_data, freeze_finbert=True):
    # Load FinBERT
    finbert_model = load_finbert_model(freeze=freeze_finbert).to(device)
    ...
    # Train the model
    train_model(model, train_loader, criterion, optimizer, num_epochs=5000)
...
def main():
    ...
    for window_name in ["filtered_7_days", "filtered_15_days", "filtered_30_days", "filtered_90_days"]:
        ...
        process_window(window_name, technical_data, textual_data, freeze_finbert=False)
